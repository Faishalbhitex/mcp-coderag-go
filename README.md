...
Skills adalah instruksi *just-in-time* yang memandu agent cara menggunakan
gocoderag secara efektif. Skills hanya diload ke context saat relevan — hemat token.

### Struktur Skills di Project Ini

```
skills/
  gocoderag/
    SKILL.md    ← instruksi penggunaan tools gocoderag
```

### Setup per AI Client

**gemini-cli** — symlink atau copy ke workspace skills:
```bash
mkdir -p .gemini/skills
ln -s ../../skills/gocoderag .gemini/skills/gocoderag
# atau copy:
cp -r skills/gocoderag .gemini/skills/
```

**qwen-code** — symlink atau copy ke .qwen/skills:
```bash
mkdir -p .qwen/skills
ln -s ../../skills/gocoderag .qwen/skills/gocoderag
```

**Portable (semua client)** — gunakan `.agents/skills/`:
```bash
mkdir -p .agents/skills
ln -s ../../skills/gocoderag .agents/skills/gocoderag
```

### Cara Kerja
Setelah skill terpasang, agent akan otomatis mengaktifkannya ketika kamu
bertanya tentang kode di project. Tidak perlu sebut "gunakan gocoderag" secara eksplisit.
